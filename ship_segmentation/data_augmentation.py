...
def add_gaussian_noise(image, mask):
    ...
    noise_stddev = np.random.uniform(0.0, 0.1)  # Random noise standard deviation

    noise = tf.random.normal(shape=tf.shape(image), mean=0.0, stddev=noise_stddev, dtype=tf.float32)
    # clipp to the range [0, 255] to ensure valid pixel values
...
def augment_and_save_data(base_dir, imgs_dir, masks_dir):
    ...
    for image_id, mask_id in zip(imgs_list, masks_list):
        ...
        plt.imsave(os.path.join(aug_images_dir, image_id), aug_image)
        plt.imsave(os.path.join(aug_masks_dir, mask_id), aug_mask, cmap='gray')
